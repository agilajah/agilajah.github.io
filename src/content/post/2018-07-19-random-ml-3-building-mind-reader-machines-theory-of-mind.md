...
However, every move taken by an agent will cost in -0.01, and -0.05 if it hits a wall. So they better be careful. That's why the agents have to plan their behaviour carefully (here, they do so through value iteration), resulting in an optimal policies πi\* with respect to their own reward functions [2]. Then, we train the ToMnet to observe these behaviours by providing an access to randomly past MDPs of each agent. With this information, the ToMnet will have to predict the agents' future behaviour. There are three settings in this particular scenario:
