...
The 'value' of an action a is the expected reward given if it is selected (see point 2 above) and denoted by $$q*(a) = E[Rt|At =a]$$. If action **At** at the time step t is on the i-th machine, then $$q*(At) = θi$$.
